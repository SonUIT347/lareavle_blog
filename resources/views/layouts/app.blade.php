        ...
        <header class="bg-blue-500 py-6">
            <div class="container mx-auto flex justify-between items-center px-6">
                ...
                <a class="no-underline hover:bg-blue-800" href="/blog">Blog</a> 
                    @guest
                        <a class="no-underline hover:bg-blue-800" href="{{ route('login') }}">{{ __('Login') }}</a>
                        @if (Route::has('register'))
                            <a class="no-underline hover:bg-blue-800" href="{{ route('register') }}">{{ __('Register') }}</a>
                        @endif
                    @else
                        <span>{{ Auth::user()->name }}</span>

                        <a href="{{ route('logout') }}"
                           class="text-red-800 no-underline hover:underline"
